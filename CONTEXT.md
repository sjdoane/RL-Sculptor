...
- **What** — the 5-lens Workflow found **2 CONFIRMED** (lenses C broad-false-grant + D honest-false-reject CLEAN — 13/13 honest accept, the dual goal-text-AND-ladder-posture loser-drop gate holds; **lens E AST escape hunt found ZERO** after a novel-numpy-2.x sweep + 8-hop BFS — the AST surface is clean a 2nd consecutive round, rounds 30–31).
  - **(A1 — HIGH, FIXED) jitter-probe fail-OPEN, the un-closed sibling of round-30 D1.** A metric that SELECTIVELY RAISES on the `jitter_in_place` probe (keyed on its mean|jv|≈1.18 band, which no competent rung occupies) left `loser_scores['jitter_in_place']` unset → `vfj=None` → the velocity-floor verdict SKIPPED (fail-OPEN), and the recorded 0.0 also dodged the ceiling. **Fix:** a jitter raise now fails CLOSED — BUT NARROWLY: only when `do_nothing_upright` was SCORED (not raised). This preserves the round-15 honest-onset case (an honest fold/onset metric that can't time a static rollout raises on BOTH jitter AND do_nothing → do_nothing absent from loser_scores → NOT flagged → not false-rejected — verified: test_round15 still green). The surgical A1 evader scores do_nothing 0.0 (doesn't raise on it) → flagged. (Residual: a 2-raise attacker who also raises on the perfectly-still do_nothing — which an honest metric scores 0, never raises on — evades; documented velocity-floor limit.)
...
- **Verified** — sculptor **942 passed / 1 skip** (+1 round-31 A1 gate-level test: jitter-raise fail-closed + an honest-peak control not flagged; round-15 honest-onset preserved), UI backend **362 passed**. uvicorn restarted (HTTP 200).
