...
- **Round 41 (harden)** in `metric_calibration.py`:
  - **(token-split farms — FIXED)** raised `_PERTURB_GOAL_DROP_MIN` 0.15 → 0.4: a granted metric must be SUBSTANTIALLY GATED on its goal joints (lose ≥0.4·comp when they are stilled), not just additively flavored. An honest goal-gated `up·goal·…` (completion_gate·min) drops to ~0 (ratio ~1.0); the token-split farms drop only ~0.26 → flagged. Catches BOTH HIGH defects (independent of the root-z on_root derivation — closes the z-axis incidental-hop hole structurally without the brittle `not goal_joints` gate that fold breaks). Honest corpus (GOOD_KICK, honest hop/fold/wave, all GATED on their goal joints) unaffected (188 calibration tests green).
