...
def _waypoint_velocity_command_types() -> tuple[type[Any], type[Any]]:
    ...
    class WaypointVelocityCommand(UniformVelocityCommand):
        ...
        def _update_command(self) -> None:
            ...
            # can be reached by arcing around the predicate, allowing command
            # convergence without objective progress. The safe cap removes
            # that bypass: every convergent path must enter the unchanged
            # predicate while the obstacle-away half-space remains preserved.
            # Objective truth is still the authored disk, which advances the
            # command on first entry.
            ...
            # low-speed equilibrium just outside the predicate. This applies
            ...
            normal_scale = torch.where(
                ...
                torch.clamp(
                    normal_scale,
                    min=float(
                        self.cfg.clearance_traversal_min_speed_scale),
                ),
                normal_scale,
            )
...
def _reconcile_waypoint_course(
    ...
    *,
    ...
    # The obstacle-away cap is inside the immutable disk. Convergence to this
    # command target therefore guarantees predicate entry, unlike an outgoing
    # target that can be approached around the disk.
    clearance_traversal_shifts = clearance_shifts
    ...
    if clearance_adjustments:
        adjustments.append(
            ...
            f"full-speed traversal to embodiment-safe cap inside frozen "
