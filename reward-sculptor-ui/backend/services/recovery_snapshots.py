...
def _matching_origin_log(
    ...
    *,
    ...
    for log_path in runs_dir.glob("_run_job_*.log"):
        ...
        effective = [
            event for event in events
            if event.get("type") == "warm_start_observation_extended"
            and event.get("effective_policy_contract")
        ]
        loaded = [
            event for event in events
            if event.get("type") == "warm_start_loaded"
            and sorted(event.get("load_cfg_keys") or []) == ["actor", "critic"]
        ]
        if not pins or not effective or not loaded:
            continue
        effective_event = effective[-1]
        try:
            event_contract_path = Path(
                str(effective_event["effective_policy_contract"])
            ).expanduser().resolve(strict=True)
        except (KeyError, OSError, RuntimeError):
            continue
        if event_contract_path != effective_contract_path:
            continue
        last_observed = max(
            (
                int(event["rl_iter"])
        ...
        candidates.append({
            ...
            "effective_policy_contract_sha256": effective_event.get(
                "effective_policy_contract_sha256"
            ),
            "warm_start_loaded": loaded[-1],
...
def _materialize_legacy_receipt(
    ...
    *,
    ...
    log_evidence = _matching_origin_log(
        ...
        iteration=iteration,
        ppo_step=ppo_step,
        effective_contract_path=effective_contract_path,
    )
    ...
    # Worker events pin the canonical contract fingerprint, not the raw JSON
    # file bytes (the sidecar is pretty-printed).  The source object itself is
    # embedded in the receipt, so this still binds every contract field.
    ...
    receipt: dict[str, Any] = {
        ...
        "source": {
            ...
            "effective_policy_contract_origin_path": str(
                effective_contract_path
            ),
...
def discover_recovery_snapshots(project_dir: Path) -> list[dict[str, Any]]:
    ...
    if recovery_root.is_dir() and not recovery_root.is_symlink():
        for recovery_dir in recovery_root.glob("snap_*"):
            if (
            ...
            rows.append(_public_summary(receipt))
    ...
    for iter_dir in runs_dir.glob("iter_*"):
        ...
        for model_path in logs_dir.glob("model_*.pt"):
            ...
            if model_match is None:
                continue
            progress = (iteration, int(model_match.group("step")))
            ...
            admitted_progress.add(progress)
            rows.append(_public_summary(receipt))
    return sorted(rows, key=lambda row: (row["iteration"], row["ppo_step"]), reverse=True)
...
def resolve_recovery_snapshot(
    ...
    *,
    ...
    effective_origin = Path(str(
        source.get("effective_policy_contract_origin_path", "")
    ))
    if effective_origin != iter_dir / "warm_start_effective_policy_contract.json":
        raise RecoverySnapshotError(
